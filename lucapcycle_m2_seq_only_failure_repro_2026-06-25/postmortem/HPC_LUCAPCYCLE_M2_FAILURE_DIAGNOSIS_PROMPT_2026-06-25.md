# HPC LucaPCycle M2 Failure Diagnosis Prompt

Date: 2026-06-25

Copy this prompt to the HPC-side AI after the vector diversity audit failed.

---

你现在在 HPC 上帮我做 **LucaPCycle M2 vector 结果失败根因诊断**。

这一步只允许做小样本诊断和只读检查；**不要训练、不要全量提取、不要提交 19 万完整任务、不要删除或覆盖已有输出、不要修改 EnzymeCAGE 训练代码、不要加载 ESM/ESM-C/ESM2**。

## 背景

当前已生成的 `107,731` 训练 UID M2 vector-only 结果结构审计通过，但数值区分度审计失败：
...
因此当前这批 vector 不能用于训练，也不能继续补跑 19 万完整酶池。

需要诊断的重点：

1. BPE/tokenizer 是否把不同序列大量编码成 `[UNK]`；
2. 我们之前手写的 tokenization 是否和 LucaPCycle 官方 `BatchConverter` 一致；
3. `seq_encoder` 的 word embedding 输出是否有差异，但 transformer 输出塌缩；
4. 是否因为没有使用 `seq_matrix` checkpoint 的 matrix/ESM 分支，导致只取 seq 分支本身不可用。
...
请在 HPC 登录节点执行下面命令。它只做小样本诊断，不需要 GPU；如果登录节点不能 import torch，可以提交一个很短的 DCU 交互/小作业，但不要跑全量。
...
1. markdown 报告路径；
2. `DIAGNOSIS_STATUS`；
3. 每个样本的 `unk_ratio`、`unique_core_ids`；
4. `manual_vs_official_ids_equal`；
5. `manual_global_unk_ratio_nonpad` 和 `official_global_unk_ratio_nonpad`；
6. `emb_mean_*_before_transformer` 的 `mean_dim_std` 和 cosine；
7. `m2_*_after_transformer` / `m2_pooler_*` 的 `mean_dim_std` 和 cosine；
8. `saved_shard0_*_first12` 的统计；
9. 报告最后的 interpretation 部分。

不要启动完整 `195,743` 酶池提取，等我根据诊断结果判断修复路线。
