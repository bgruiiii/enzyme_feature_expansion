# HPC LucaPCycle M2-Only Asset Audit Prompt

Date: 2026-06-25

Copy this prompt to the HPC-side AI only if we need to conclusively check
whether the current HPC LucaPCycle assets contain an M2-only route.

---

你现在在 HPC 上帮我做 **LucaPCycle M2-only 资产审计**。

目标只回答一个问题：

```text
当前 HPC 上是否存在官方 M2-only / seq-only checkpoint 或官方 M2-only
导出入口，可以不加载/不生成 ESM/ESM2 matrix，直接得到可用 M2 特征？
```
...
```text
列目录。
读取 README / 脚本 / config.json / training_args.bin 的轻量元信息。
用 Python torch.load(..., map_location="cpu") 读取 training_args.bin 可以，
但不要 load pytorch_model.bin。
在限定目录内 find/grep，不要全盘搜索。
生成 markdown 审计报告。
```
...
## Return To Local Chat

请把完整 markdown 报告贴回来，尤其是：

```text
REPORT_WRITTEN
M2_ONLY_ASSET_AUDIT_STATUS
m2_only_checkpoint_paths
seq_matrix_checkpoint_paths
official_m2_only_export_entrypoint
requires_esm_or_matrix_for_official_prediction
recommendation
```
