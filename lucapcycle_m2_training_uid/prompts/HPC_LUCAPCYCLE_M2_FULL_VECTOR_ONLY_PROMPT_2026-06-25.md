# HPC LucaPCycle M2 Full Vector-Only Extraction Prompt

Date: 2026-06-25

Copy this prompt to the HPC-side AI only after the 10-UID sample extraction and
sample output audit have both passed.

---

你现在在 HPC 上帮我做 **LucaPCycle M2 全量 vector-only 特征提取**。

这一步只提取训练用的固定长度 vector 特征，不保存全量 token-level matrix。完整矩阵体积很大，后续如果老师明确需要，再单独开 token-matrix 提取分支重新跑。
...
- 只提取 vector 特征。
- 不保存全量 token-level matrix。
- 不训练。
- 不运行 `prediction_v2.py` 做分类预测。
- 不加载 ESM/ESM-C/ESM2。
- 不安装 `esm`、`fair-esm`、`deepspeed`、`tensorflow`。
- 不修改 EnzymeCAGE 训练代码。
- 如作业中断，依靠 manifest 断点续跑；不要删除已有 shard。

M2 不需要 ESM。路线是：

```text
sequence -> BPE/subword tokens -> LucaPCycle seq_encoder -> M2 -> pooled vectors
```
...
$OUT_ROOT/
  ...
  vector_features/
    ...
    m2_vectors_part_000001.npz
    ...
  logs/
    lucapcycle_m2_full_vector_<JOBID>.out
    lucapcycle_m2_full_vector_<JOBID>.err
    lucapcycle_m2_full_vector_report_<JOBID>.md
```

每个 vector shard `.npz` 必须包含：
...
m2_mean                  shape: (N, 1024), float32
m2_max                   shape: (N, 1024), float32
m2_value_attention       shape: (N, 1024), float32
m2_projected_256         shape: (N, 256), float32
```

## 全量提取策略

- 输入：训练表中去重后的 `UniprotID,sequence`。
- 预期唯一 UID 数：约 `107,731`。
- 默认 batch size：`4`，优先稳。
- 默认 shard size：`1000` UID/shard。
- 断点续跑：
  - 如果 `$OUT_ROOT/uid_to_shard.csv` 已存在，读取其中 UID 作为已完成；
  - 跳过已完成 UID；
  - 新 shard 写完后再追加 manifest；
  - 不要覆盖已有 shard。
- 失败策略：
  - 单个 UID 失败时写入 `failed_uids.csv`；
  - 不因单个 UID 失败停止全量；
  - 模型加载、checkpoint key 异常、vector shape 异常必须停止。
- checkpoint key 策略：
  - 只允许白名单过滤 `loss_fct.pos_weight`；
  - 任何其他 unexpected key 或 missing key 都必须停止。

## 请运行的命令

请在 HPC 上执行下面这段命令。若分区名、DCU 资源参数或 walltime 不适合当前集群，只允许调整 `#SBATCH --partition`、`#SBATCH --gres`、`#SBATCH --time`。
...
作业可能较久。请等 SLURM 作业结束后，把下面内容贴回来：

1. markdown 报告路径；
2. SLURM job id；
3. `.out` 和 `.err` 文件路径；
4. `FULL_VECTOR_STATUS`；
5. `total_unique_input`；
6. `completed_total`；
7. `failed_total`；
8. `shard_file_count`；
9. `processed_new_this_run`；
10. `filtered_allowed_keys`；
11. 如果 `.err` 非空，贴出前 120 行；
12. 如果状态是 `INCOMPLETE`，不要删除输出，直接告诉我，我们会用同一脚本续跑。

不要启动任何 token-level matrix 全量提取，等我审核 vector-only 全量结果。
