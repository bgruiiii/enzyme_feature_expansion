# HPC LucaPCycle M2 Full Vector Output Audit Prompt

Date: 2026-06-25

Copy this prompt to the HPC-side AI after the full vector-only extraction report
shows `FULL_VECTOR_STATUS=PASS`.

---

你现在在 HPC 上帮我做 **LucaPCycle M2 全量 vector-only 输出结果审计**。

这一步只允许读取和检查已经生成的结果文件，生成 markdown 审计报告；**不要训练、不要重新提取、不要提交 SLURM、不要修改 EnzymeCAGE 训练代码、不要加载 ESM/ESM-C/ESM2、不要删除或覆盖已有输出**。

## 背景

全量 vector-only 提取已经完成，报告显示：
...
- 结果目录是否完整；
- manifest 和 shard 是否能对齐；
- `.npz` 文件内部的数据结构、key、shape、dtype；
- 107,731 条 UID 是否全部覆盖且无重复；
- 是否有 NaN/Inf；
- `.err` 是否为空；
- 输出示例长什么样。
...
JOB_ID=115940591
RUN_REPORT=/public/home/acfbwjsi7s/lucapcycle_m2_features_2026-06-25_vector_only/logs/lucapcycle_m2_full_vector_report_115940591.md
RUN_OUT=/public/home/acfbwjsi7s/lucapcycle_m2_features_2026-06-25_vector_only/logs/lucapcycle_m2_full_vector_115940591.out
RUN_ERR=/public/home/acfbwjsi7s/lucapcycle_m2_features_2026-06-25_vector_only/logs/lucapcycle_m2_full_vector_115940591.err
```

## 请运行的命令

请在 HPC 登录节点执行下面这段命令即可。它是只读审计，不需要 GPU/DCU，不提交 SLURM。
...
1. markdown 审计报告路径；
2. `FULL_VECTOR_OUTPUT_AUDIT_STATUS`；
3. `error_count`；
4. `RUN_ERR_BYTES`，如果非 0，把前 120 行也贴回来；
5. 输出目录总大小和 `vector_features` 大小；
6. `shard_count`、`total_rows_across_shards`、`row_count_distribution`；
7. `input_manifest_rows`、`uid_to_shard_rows`、UID 重复数、缺失数、额外数；
8. 三个被抽样 shard 的 key、shape、dtype；
9. 3-5 条示例 UID 记录，包括 UID、sequence_length、token_count、三个 feature 的 first5 和 L2 norm；
10. `Teacher-Facing Feature Schema` 那一段。

如果审计失败，不要删除任何输出，把失败报告原样发回来。
